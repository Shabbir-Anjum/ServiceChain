...
const rowToRec = (r) => ({
  ...
  proofPhotoUrl: r.proof_photo_url || null,
  aiCheck: r.ai_check || null,
  clientDecisionResult: r.client_decision || null,
...
async function update(uuid, patch) {
  if (await dbReady()) {
    ...
    if (patch.proofText !== undefined) row.proof_text = patch.proofText;
    if (patch.proofPhotoUrl !== undefined) row.proof_photo_url = patch.proofPhotoUrl;
    if (patch.aiCheck !== undefined) row.ai_check = patch.aiCheck;
    // Fold the on-chain verdict into ai_check.onchain (no extra DB column needed).
    if (patch.onchainCheck !== undefined && patch.aiCheck !== undefined && patch.aiCheck) {
      row.ai_check = { ...patch.aiCheck, onchain: patch.onchainCheck };
    }
    if (patch.proofSubmittedAt !== undefined) row.proof_submitted_at = patch.proofSubmittedAt;
    if (patch.clientDecisionResult !== undefined) row.client_decision = patch.clientDecisionResult;
    if (patch.clientDecidedAt !== undefined) row.client_decided_at = patch.clientDecidedAt;
