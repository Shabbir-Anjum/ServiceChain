...
function WorkersAdmin() {
  ...
  const [form, setForm] = useState({ name: "", role: "", emoji: "", skills: "", location: "", tagline: "", rating: "", wallet: "" });
  ...
  async function addWorker(e) {
    ...
    try {
      ...
      else { setMsg({ type: "ok", text: `Added ${d.worker.name} (${d.worker.id})` }); setForm({ name: "", role: "", emoji: "", skills: "", location: "", tagline: "", rating: "", wallet: "" }); load(); }
  ...
  return (
    <div className="admin__grid">
      <form className="glass-card glass-card--accent" onSubmit={addWorker} aria-busy={busy}>
        <h3 style={{ marginBottom: "var(--s-3)" }}>Add a worker</h3>
        <div className="stack gap-3">
          ...
          <div className="row gap-3">
            <div className="field" style={{ width: 110 }}><label className="label">Rating</label><input className="input" type="number" step="0.1" min="0" max="5" value={form.rating} onChange={set("rating")} placeholder="4.8" /></div>
            <div className="field" style={{ flex: 1 }}><label className="label">Wallet (optional)</label><input className="input mono" value={form.wallet} onChange={set("wallet")} placeholder="0x…" /></div>
          </div>
        </div>
        {msg && <div className={`pill ${msg.type === "ok" ? "is-success" : "is-danger"}`} style={{ marginTop: "var(--s-3)" }}>{msg.text}</div>}
        <button type="submit" className="btn btn-primary btn-block" style={{ marginTop: "var(--s-4)" }} disabled={busy}>{busy ? <><span className="spinner" /> Adding…</> : "Add worker"}</button>
      </form>

      <div>
        <h3 style={{ marginBottom: "var(--s-3)" }}>Marketplace ({workers?.length ?? "…"})</h3>
        {workers === null ? <div className="skeleton" style={{ height: 300, borderRadius: 16 }} /> : (
          <div className="stack gap-2" style={{ maxHeight: 520, overflow: "auto" }}>
            {workers.map((w) => (
              <div key={w.id} className="glass-card" style={{ padding: "var(--s-3)" }}>
                <div className="row gap-3">
                  <div className="wkr__avatar" aria-hidden="true">{w.emoji}</div>
                  <div className="stack" style={{ flex: 1 }}>
                    <strong>{w.name} <span className="dim mono" style={{ fontSize: ".75rem" }}>{w.id}</span></strong>
                    <span className="dim" style={{ fontSize: ".82rem" }}>{w.role} · {w.location} · ★ {w.rating}</span>
                  </div>
                  <span className={`pill ${w.available ? "is-success" : "is-pending"}`} style={{ fontSize: ".7rem" }}>{w.available ? "Available" : "Busy"}</span>
                </div>
              </div>
            ))}
          </div>
